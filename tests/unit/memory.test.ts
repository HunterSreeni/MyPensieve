...
describe("MemoryIndex (SQLite)", () => {
	...
	it("searches decisions by text", () => {
		index.indexDecision({
			...
			session_id: "s1",
			project: "test",
			content: "Use SQLite for indexing",
			confidence: 0.95,
			source: "manual",
			tags: [],
		});
	...
	it("indexes and queries threads", () => {
		index.indexThread({
			...
			status: "open",
			messages: [
				{ timestamp: "2026-04-10T12:00:00Z", session_id: "s1", role: "operator", content: "msg" },
			],
			tags: [],
		});
...
describe("MemoryQuery (unified recall)", () => {
	...
	it("searches across all layers", () => {
		decisions.addDecision({
			sessionId: "s1",
			project: "test",
			content: "Use SQLite for memory",
			confidence: 0.95,
			source: "manual",
		});
		threads.createThread({
			project: "test",
			title: "SQLite vs Redis discussion",
			firstMessage: {
				timestamp: "2026-04-10T12:00:00Z",
				session_id: "s1",
				role: "operator",
				content: "thinking about SQLite",
			},
		});
		persona.addDelta({
			sessionId: "s1",
			field: "tech_preference",
			deltaType: "add",
	...
	it("respects limit", () => {
		for (let i = 0; i < 10; i++) {
			decisions.addDecision({
				sessionId: "s1",
				project: "test",
				content: `decision ${i} about config`,
				confidence: 0.8,
				source: "auto",
			});
		}
